...
def compute_cost(AL, Y): # TODO: TEST
...
def L_layer_model(X,Y, layers_dims, learning_rate, num_iterations, batch_size):
    ...
    # init values for the iterations
    ...
    # init parameters (wieghts and biases)
    ...
    for epoch in range(num_iterations):
        for i in range(batch_size, X.shape[1], batch_size):
            ...
            # calc training cost
            cost = compute_cost(AL, Yi)
            

            # evaluate validation set (cost & acc)
            val_acc = predict(x_validation_set, y_validation_set, (params))
            Y_val_pred, _ = L_model_forward(x_validation_set, (params), use_batchnorm= True) # the error maybe becuase of batchnorm
            val_cost = compute_cost(Y_val_pred, y_validation_set)
    ...
    cost = compute_cost(AT, Y)
    
    # log
    print(f"Training Acc: {predict(X, Y , params)}, all train cost: {cost}")

    return params, costs, val_losses


def predict(X,Y, parameters):
    Y_pred, caches = L_model_forward(X, parameters, use_batchnorm= True)

    # get the index of the highest value of each sampe and compare
